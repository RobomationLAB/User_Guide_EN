...
The NOT block **exceptionally** considered to have a **true** value if no input is provided.  
For example, the block below returns **false**.
...
The circular slider controls the Red (R), Green (G), and Blue (B) values, and the bar slider can be used to adjust the **brightness**.
...
## Keypressed
The **Keypressed** block returns true / false whether a specific keyboard key is pressed.  
This block can be configured to perform an action when a specific key is pressed.  

![Image](https://github.com/user-attachments/assets/d6ca31c2-b8e0-4ee6-998b-7eef2b53dea9)

### Javascript Code
```javascript
__keypressed(32)  // true when spacebar(32) is pressed, else false
```
...
When used with `and` blocks, you can detect multiple keypress simultaneously.

![Image](https://github.com/user-attachments/assets/c39922c0-866c-4a3f-8e10-d67c1ba1e929)

### Javascript Code
```javascript
if (__keypressed(32) && __keypressed(16)) {  // When Space(32) and Shift(16) are pressed simulateneously
    window.alert('Keypress event');
}
```
...
### Python Code
```python
if __keypressed(32) and __keypressed(16):  # When Space(32) and Shift(16) are pressed simulateneously
    print('Keypress event')
```
...
If the variable is not declared in the code, declare a new variable and assign a value to it.  
